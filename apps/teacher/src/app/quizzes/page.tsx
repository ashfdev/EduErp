...
export default function TeacherQuizzesPage() {
  ...
  const createQuestionMutation = useMutation({
    ...
    onError: (err: unknown) => toast.error((err as { response?: { data?: { error?: { message?: string } } } })?.response?.data?.error?.message ?? "Failed to add question"),
  });
  ...
  return (
    <TeacherShell>
      <PageWrapper className="p-0">
        <PageHeader title="Quizzes / Online Exams" subtitle="Question bank, timed quizzes, and auto-graded results" />
        ...
        {subjectId && (
          <>
            <div className="flex items-center justify-between">
              <p className="text-sm font-medium">Question Bank ({questions?.length ?? 0})</p>
              <Button size="sm" variant="outline" onClick={() => setQOpen(true)}>+ Add Question</Button>
            </div>
            <Card>
              <CardContent className="pt-6">
                {!questions?.length && <EmptyState title="No questions yet" />}
                <div className="space-y-2">
                  {questions?.map((q) => (
                    <div key={q.id} className="rounded-md border p-3 text-sm">
                      ...
                      <p className="text-xs text-muted-foreground">{q.options.map((o) => `${o.key}) ${o.text}`).join(" · ")} — correct: {q.correct_option}</p>
                    </div>
                  ))}
                </div>
              </CardContent>
            ...
            <div className="flex items-center justify-between">
              ...
              <Button size="sm" onClick={() => setQuizOpen(true)} disabled={!questions?.length}>+ Create Quiz</Button>
            </div>
            <Card>
              <CardContent className="pt-6">
                {!quizzes?.length && <EmptyState title="No quizzes yet" />}
                <div className="space-y-2">
                  {quizzes?.map((q) => (
                    <div key={q.id} className="flex items-center justify-between rounded-md border p-3 text-sm">
                      <div>
                        <p className="font-medium">{q.title}</p>
                        <p className="text-xs text-muted-foreground">{q._count.questions} questions · {q.duration_minutes} min · {q._count.attempts} attempt(s)</p>
                      </div>
                      <div className="flex items-center gap-2">
                        <Badge variant={q.is_published ? "default" : "outline"}>{q.is_published ? "Published" : "Draft"}</Badge>
                        {!q.is_published && <Button size="sm" variant="outline" onClick={() => publishMutation.mutate(q.id)}>Publish</Button>}
                        <Button size="sm" variant="outline" onClick={() => setResultsQuizId(q.id)}>Results</Button>
                      </div>
                    </div>
                  ))}
                </div>
      ...
      <Dialog open={qOpen} onOpenChange={setQOpen}>
        <DialogContent className="max-w-lg">
          ...
          <div className="space-y-3">
            <div className="space-y-1.5"><Label>Question</Label><Input value={qText} onChange={(e) => setQText(e.target.value)} /></div>
            {qOptions.map((o, i) => (
              <div key={i} className="flex items-center gap-2">
                ...
                <label className="flex items-center gap-1 text-xs">
                  <input type="radio" checked={qCorrect === o.key} onChange={() => setQCorrect(o.key)} /> Correct
                </label>
              </div>
            ))}
            <Button size="sm" variant="outline" onClick={() => setQOptions((prev) => [...prev, { key: slugKey(`opt${prev.length}`) + prev.length, text: "" }])}>+ Add Option</Button>
            <div className="space-y-1.5"><Label>Marks</Label><Input type="number" className="w-24" value={qMarks} onChange={(e) => setQMarks(Number(e.target.value))} /></div>
          </div>
          <DialogFooter>
      ...
      <Dialog open={quizOpen} onOpenChange={setQuizOpen}>
        <DialogContent className="max-w-lg">
          ...
          <div className="space-y-3">
            <div className="space-y-1.5"><Label>Title</Label><Input value={quizTitle} onChange={(e) => setQuizTitle(e.target.value)} /></div>
            <div className="space-y-1.5"><Label>Duration (minutes)</Label><Input type="number" className="w-24" value={duration} onChange={(e) => setDuration(Number(e.target.value))} /></div>
          ...
          <div className="space-y-2">
            {attempts?.map((a) => (
              <div key={a.id} className="flex items-center justify-between rounded-md border p-2 text-sm">
                ...
                <div className="flex items-center gap-2">
                  {a.tamper_flag_count > 0 && <Badge variant="outline">⚠ {a.tamper_flag_count} flag(s)</Badge>}
