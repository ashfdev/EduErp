...
export default function PortalLoginPage() {
  ...
  const { data: institution } = useQuery<{ logo_url: string | null; name_en: string; student_login_bg_url: string | null }>({
  ...
  return (
    <div className="relative flex min-h-screen w-full bg-white">
      ...
      <div 
        ...
        style={{
          backgroundImage: `url('${institution?.student_login_bg_url ?? "/assets/login-illustration.png"}')`,
        }}
      />
          ...
          <div className="mx-auto w-full max-w-md rounded-2xl bg-white p-8 shadow-sm border border-slate-100">
            <form onSubmit={submit} className="space-y-6">
              <div className="space-y-2">
                ...
                <Input className="rounded-lg bg-[#f8fafc] px-4 py-6 text-sm transition-all focus:bg-white" value={identifier} onChange={(e) => setIdentifier(e.target.value)} required autoFocus placeholder={t("idPlaceholder")} />
              </div>
