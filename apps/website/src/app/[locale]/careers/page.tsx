        ...
        <div className="grid grid-cols-1 gap-5">
          {jobs.map((job) => (
            <Link 
              key={job.id} 
              href={`/careers/${job.id}`} 
              className="group relative flex flex-col sm:flex-row sm:items-center justify-between gap-5 rounded-3xl bg-gradient-to-br from-green-50/80 via-white to-emerald-50/80 p-6 sm:p-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_8px_40px_rgb(34,197,94,0.15)] hover:border-green-100"
            >
              <div className="flex-1">
                <h2 className="text-xl font-extrabold text-slate-900 group-hover:text-primary transition-colors mb-3">
                  {job.title}
                </h2>
                <div className="flex flex-wrap items-center gap-3 text-xs mb-4">
                  <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-white text-green-700 font-bold ring-1 ring-slate-100 shadow-sm">
              ...
              <div className="shrink-0 pt-4 sm:pt-0">
                <div className="flex h-14 w-14 items-center justify-center rounded-full bg-white text-primary ring-1 ring-slate-100 shadow-sm group-hover:bg-primary group-hover:text-white group-hover:ring-primary/50 transition-all duration-300 transform group-hover:scale-110 group-hover:shadow-lg">
