...
// Event.type is a free-form string (no schema enum, to avoid a breaking
// migration on existing rows) — only a small known set gets a real color;
// anything else falls back to neutral gray rather than erroring.
...
export const KNOWN_EVENT_TYPES = Object.keys(TYPE_COLORS);
...
export function MonthCalendar({ year, month, events, onEventClick }: MonthCalendarProps) {
        ...
        <div className="grid grid-cols-7">
          {cells.map((d, i) => {
            ...
            return (
              <div
                key={i}
                className={`min-h-[92px] border-b border-r p-1.5 ${inMonth ? "bg-white" : "bg-gray-50"} ${i % 7 === 6 ? "border-r-0" : ""}`}
              >
