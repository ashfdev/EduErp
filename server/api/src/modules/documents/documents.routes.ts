...
documentsRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    // Clearance resolution and any override's AdmitCardOverride/audit-log
    // writes above always run synchronously regardless of batch size --
    // only the (expensive) PDF render itself is threshold-routed, via the
    // exact same builder (buildAdmitCardsClassJob) the background worker
    // calls for a genuinely large batch. The builder receives the already-
    // resolved `included` student ids and re-derives everything else
    // (subject configs, seat plans, schedules) from live data itself.
    const studentIds = included.map((s) => s.id);
    const params = { exam_id: examId, class_id: query.class_id, student_ids: studentIds };
    if (studentIds.length <= BATCH_JOB_THRESHOLD) {
      const { buffer, filename } = await buildAdmitCardsClassJob(params);
      sendPdf(res, buffer, filename, query.download === "true");
      return;
    }
    const job = await enqueueBatchJob("ADMIT_CARDS_CLASS", params, req.user!.sub);
...
// (jobs/document-batch.job.ts) call the exact same function via the shared
// registry (lib/batch-job-registry.ts), so the two paths can never produce
// different output for the same input. That same registry is also where
// every non-PDF batch export (Excel/CSV, one per owning module) registers
// its own kind -- this file only owns the PDF-shaped kinds registered near
...
// Deliberately NOT symmetric with the other builders above: clearance
// resolution (which students are cleared vs. blocked, and any override's
// AdmitCardOverride row + audit log entry) is a real business decision the
// caller makes at request time -- it always runs synchronously in the route
// handler below, regardless of batch size, since it's cheap (a handful of
// DB writes, not Puppeteer rendering) and must never be silently re-decided
// later by a worker that could see different clearance state by the time it
// runs. This builder only ever receives the already-resolved student_ids to
// include -- it re-fetches full student records for rendering, but performs
// no clearance logic and writes nothing.
export async function buildAdmitCardsClassJob(params: { exam_id: string; class_id: string; student_ids: string[] }): Promise<BatchJobResult> {
  const { exam_id: examId, class_id: classId, student_ids: studentIds } = params;
  const exam = await prisma.exam.findUnique({ where: { id: examId } });
  if (!exam) throw notFound("Exam not found");
  ...
  if (!students.length) throw badRequest("No students to include in this admit-card batch");
  ...
  const dataList = await Promise.all(
    students.map(async (student) => {
      ...
      // See the identical eligibility-scoping note on the synchronous route
      // handler this builder was extracted from -- a shared (group_id: null)
      // subject applies to everyone in the student's own class; a
      // group-scoped one only to students in that same group, and never to
      // a different class's ungrouped subject.
  ...
  const buffer = await renderDocumentBatch("ADMIT_CARD", dataList as unknown as Record<string, unknown>[]);
  return { buffer, filename: `admit-cards-${classId}.pdf` };
}

export async function buildStaffIdCardsAllJob(): Promise<BatchJobResult> {
  ...
  const buffer = await renderDocumentBatch("STAFF_ID_CARD", staffList.map((staff) => ({ staff })), { pageSize: "ID_CARD" });
  return { buffer, filename: "id-cards-all-staff.pdf" };
}
...
// Every kind registers itself into the shared registry (lib/batch-job-
// registry.ts) once, at module load -- the synchronous route handlers below
// call each build function directly; the background worker (jobs/document-
// batch.job.ts) resolves the same function via getBatchJobBuilder(job_kind),
// so the two paths can never drift.
registerBatchJobKind("ID_CARDS_CLASS", "STUDENT_ID_CARD", (params) => buildIdCardsClassJob(params as { class_id: string }));
registerBatchJobKind("MARKSHEETS_CLASS", "MARKSHEET", (params) =>
  buildMarksheetsClassJob(params as { exam_id: string; class_id: string; group_id?: string; section_id?: string }),
);
registerBatchJobKind("TABULATION_SHEET", "TABULATION_SHEET", (params) =>
  buildTabulationSheetJob(params as { exam_id: string; class_id: string; group_id?: string; section_id?: string }),
);
registerBatchJobKind("BLANK_MARKSHEET", "BLANK_MARKSHEET", (params) =>
  buildBlankMarksheetJob(params as { exam_id: string; class_id: string; group_id?: string; section_id?: string }),
);
registerBatchJobKind("MERIT_LIST", "MERIT_LIST", (params) =>
  buildMeritListJob(params as { exam_id: string; class_id: string; group_id?: string; section_id?: string }),
);
registerBatchJobKind("ATTENDANCE_MONTHLY_SHEET", "ATTENDANCE_SHEET", (params) =>
  buildAttendanceMonthlySheetJob(params as { class_id: string; section_id?: string; month: number; year: number }),
);
registerBatchJobKind("ATTENDANCE_BLANK_SHEET", "ATTENDANCE_BLANK", (params) =>
  buildAttendanceBlankSheetJob(params as { class_id: string; section_id?: string; from_date: string; to_date: string }),
);
registerBatchJobKind("ADMIT_CARDS_CLASS", "ADMIT_CARD", (params) =>
  buildAdmitCardsClassJob(params as { exam_id: string; class_id: string; student_ids: string[] }),
);
registerBatchJobKind("STAFF_ID_CARDS_ALL", "STAFF_ID_CARD", () => buildStaffIdCardsAllJob());
...
documentsRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const job = await enqueueBatchJob("MARKSHEETS_CLASS", params, req.user!.sub);
    res.status(202).json({ success: true, data: { job_id: job.id, status: job.status } });
  }),
);
...
documentsRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const job = await enqueueBatchJob("TABULATION_SHEET", params, req.user!.sub);
    res.status(202).json({ success: true, data: { job_id: job.id, status: job.status } });
  }),
);
...
documentsRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const job = await enqueueBatchJob("BLANK_MARKSHEET", params, req.user!.sub);
    res.status(202).json({ success: true, data: { job_id: job.id, status: job.status } });
  }),
);
...
documentsRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const job = await enqueueBatchJob("MERIT_LIST", params, req.user!.sub);
    res.status(202).json({ success: true, data: { job_id: job.id, status: job.status } });
  }),
);
...
documentsRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const job = await enqueueBatchJob("ATTENDANCE_MONTHLY_SHEET", query, req.user!.sub);
    res.status(202).json({ success: true, data: { job_id: job.id, status: job.status } });
  }),
);
...
documentsRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const job = await enqueueBatchJob("ATTENDANCE_BLANK_SHEET", query, req.user!.sub);
    res.status(202).json({ success: true, data: { job_id: job.id, status: job.status } });
  }),
);
...
documentsRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const job = await enqueueBatchJob("ID_CARDS_CLASS", { class_id: classId }, req.user!.sub);
    res.status(202).json({ success: true, data: { job_id: job.id, status: job.status } });
  }),
);
...
documentsRouter.get(
  "/id-cards/all-staff",
  asyncHandler(async (req, res) => {
    const count = await prisma.staff.count({ where: { is_active: true, deleted_at: null } });
    if (!count) throw badRequest("No active staff found");
    if (count <= BATCH_JOB_THRESHOLD) {
      const { buffer, filename } = await buildStaffIdCardsAllJob();
      sendPdf(res, buffer, filename, req.query.download === "true");
      return;
    }
    const job = await enqueueBatchJob("STAFF_ID_CARDS_ALL", {}, req.user!.sub);
    res.status(202).json({ success: true, data: { job_id: job.id, status: job.status } });
  }),
);
